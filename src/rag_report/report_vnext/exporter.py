...
from src.rag_report.report_vnext.models import IntroMetricPack, IntroRenderBundle, MetricRecord
...
class VNextHTMLExporter:
    ...
    def _metric_tooltip(self, record: MetricRecord) -> str:
        ...
        for source in record.input_sources:
            ...
            rows.append(
                ...
                f"<span class='metric-subtle'>{self._escape(translate_unaccented_text(source.canonical_line_item))} | {self._escape(source_label)}</span>"
                "</li>"
            )
        ...
        return (
            ...
            "<span class='metric-tooltip-section'><strong>Công thức</strong>"
            f"<span class='metric-formula'>{formula}</span>"
            f"<span class='metric-subtle'>Nguồn công thức: {self._escape(source_summary)}</span>"
            f"<span class='metric-subtle'>{self._escape(explanation)}</span>"
            "</span>"
            "<span class='metric-tooltip-section'><strong>Số liệu dùng để tính</strong>"
            f"<ul>{''.join(rows) if rows else '<li>Không có input.</li>'}</ul>"
            "</span>"
            "<span class='metric-tooltip-section'><strong>Ghi chú</strong>"
            f"<ul>{notes}</ul>"
            "</span>"
            f"{gap}"
            "</span>"
    ...
    def _render_chart_figure(self, index: int, item: Any, spec: Any) -> tuple[str, str] | None:
        ...
        source_footer = "Nguồn: " + "; ".join(sources[:3]) if sources else "Nguồn: dữ liệu tổng hợp A32"
        dom_id = f"chart-{item.chart_id}"
        figure_id = f"figure-{item.chart_id}"
        figure_style = "style='--figure-min-height: 460px; --chart-height: 320px;'"
    ...
    def compile_report(self, bundle: IntroRenderBundle, output_filename: str | None = None) -> str:
        ...
        narrative_html = self._markdown_to_html(bundle.narrative.markdown)
        audit_html = self._render_audit_summary(bundle)
        metric_table_html = self._render_metric_table(bundle.metric_pack)
        charts_html, chart_js = self._render_chart_pages(bundle)
        notes_html = self._render_notes(bundle.narrative.data_gaps)
        references_html = self._render_references()

        # Build verdict panel HTML dynamically
        verdict_text = bundle.narrative.verdict or ""
        reliability = bundle.narrative.verdict_source_reliability or ""
        eq_2025 = bundle.narrative.verdict_earnings_quality_2025 or ""
        liq_short = bundle.narrative.verdict_liquidity_short_term or ""
        deep_check = bundle.narrative.verdict_needs_deep_check or ""

        def _verdict_flag_class(flag_text: str) -> str:
            norm = flag_text.lower()
            if "cảnh báo cao" in norm or "nguy cơ" in norm or "cao" in norm:
                return "verdict-flag flag-red"
            if "cảnh báo" in norm or "trung bình" in norm or "khá" in norm:
                return "verdict-flag flag-yellow"
            if "tốt" in norm or "an toàn" in norm or "xanh" in norm:
                return "verdict-flag flag-green"
            return "verdict-flag"
        ...
        if verdict_text:
            ...
            <div class="verdict-panel">
              ...
              <div class="verdict-text">{self._escape(verdict_text)}</div>
              <div class="verdict-grid">
                <div class="verdict-item">Độ tin cậy nguồn: <span class="{_verdict_flag_class(reliability)}">{self._escape(reliability)}</span></div>
                <div class="verdict-item">Chất lượng lợi nhuận 2025: <span class="{_verdict_flag_class(eq_2025)}">{self._escape(eq_2025)}</span></div>
                <div class="verdict-item">Thanh khoản/ngắn hạn: <span class="{_verdict_flag_class(liq_short)}">{self._escape(liq_short)}</span></div>
                <div class="verdict-item">Cần kiểm tra sâu: <strong>{self._escape(deep_check)}</strong></div>
              </div>
            </div>
            """
        ...
        <div class="glossary-box">
          <div class="glossary-title">Đọc nhanh các thuật ngữ</div>
          <table class="glossary-table">
            ...
            <tbody>
              <tr>
                <td><strong>LNST</strong></td>
                <td>Lợi nhuận sau thuế, tức phần lãi kế toán còn lại sau khi trừ chi phí và thuế.</td>
              </tr>
              <tr>
                <td><strong>CFO</strong></td>
                <td>Dòng tiền từ hoạt động kinh doanh, tức tiền thật tạo ra từ hoạt động chính.</td>
              </tr>
              <tr>
                <td><strong>CFO/LNST</strong></td>
                <td>Kiểm tra 1 đồng lợi nhuận kế toán có tạo ra bao nhiêu đồng tiền thật.</td>
              </tr>
              <tr>
                <td><strong>Accrual Ratio</strong></td>
                <td>Đo mức lợi nhuận phụ thuộc vào ghi nhận kế toán thay vì tiền thực thu.</td>
              </tr>
              <tr>
                <td><strong>DSRI</strong></td>
                <td>Kiểm tra phải thu có tăng nhanh hơn doanh thu không.</td>
              </tr>
              <tr>
                <td><strong>Cash Buffer</strong></td>
                <td>Đệm tiền mặt so với nợ ngắn hạn. Số càng thấp thì càng ít dư địa thanh khoản.</td>
              </tr>
              <tr>
                <td><strong>Dòng tiền tự do sau cổ tức</strong></td>
                <td>Tiền còn lại sau hoạt động kinh doanh, đầu tư duy trì và trả cổ tức. Âm sâu là dấu hiệu căng tiền.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
<head>
  ...
  <title>A32 Financial Report vNext</title>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .audit-table th, .indicator-table th {{
      background: #f7f3ec;
      font-size: .8rem;
      letter-spacing: .04em;
      text-transform: uppercase;
    }}
    .metric-row.flag-red {{ background: linear-gradient(90deg, rgba(185,28,28,0.04), transparent 28%); }}
    .metric-row.flag-yellow {{ background: linear-gradient(90deg, rgba(180,83,9,0.04), transparent 28%); }}
    .metric-row.flag-green {{ background: linear-gradient(90deg, rgba(15,118,110,0.04), transparent 28%); }}
    .metric-name-cell {{ min-width: 240px; }}
    .metric-note {{ color: var(--muted); margin-top: 6px; font-size: .84rem; }}
    .metric-formula-cell {{ white-space: nowrap; }}
    .metric-value-cell, .metric-flag-cell {{ white-space: nowrap; }}
    .metric-hover-anchor {{
      position: relative;
    ...
    .metric-tooltip {{
      ...
      border-radius: 16px;
      padding: 14px 16px;
      box-shadow: 0 18px 36px rgba(8, 15, 31, 0.24);
      z-index: 20;
    }}
    .metric-hover-anchor:hover .metric-tooltip {{
      display: block;
    }}
    .metric-tooltip-section + .metric-tooltip-section {{ margin-top: 10px; }}
    .metric-tooltip p, .metric-tooltip li, .metric-tooltip span {{
      margin: 4px 0;
      font-size: .86rem;
      line-height: 1.5;
    }}
    .metric-subtle {{ color: #c7d4e6; font-size: .8rem; }}
    .metric-gap {{ color: #fca5a5; font-weight: 700; }}
    .metric-formula {{ font-family: "Times New Roman", serif; font-size: .98rem; }}
    .figure-grid {{
      display: grid;
    ...
    .report-figure {{
      ...
      border: 1px solid var(--line);
      border-radius: 8px;
      min-height: var(--figure-min-height, 460px);
      break-inside: avoid;
    ...
    .chart-mount {{
      ...
      flex: 0 0 var(--chart-height, 280px);
    }}
    .figure-caption {{
      margin-top: 10px;
      color: var(--ink);
    ...
    .verdict-flag.flag-red {{
      background: #fef2f2;
      color: #dc2626;
      border: 1px solid #fca5a5;
    }}
    .verdict-flag.flag-yellow {{
      background: #fffbeb;
      color: #d97706;
      border: 1px solid #fde68a;
    }}
    .verdict-flag.flag-green {{
      background: #f0fdf4;
      color: #16a34a;
...
<body>
  <main class="report-shell">
    <section class="report-page">
      <div class="section-kicker">A32 Financial Report vNext</div>
      <h1 class="cover-title">{self._escape(bundle.narrative.title)}</h1>
      <p class="cover-subtitle">Kiểm tra độ tin cậy của số liệu qua kiểm toán, dòng tiền, công nợ và khả năng thanh khoản.</p>
      <div class="page-footer">Báo cáo mở đầu · bản xuất HTML</div>
    </section>

    <section class="report-page">
      <div class="section-kicker">Tường thuật</div>
      <h2 class="section-title">Phần mở đầu</h2>
      <article class="report-section prose-panel">
        {verdict_html}
        {narrative_html}
      </article>
      {notes_html}
    ...
    <section class="report-page">
      ...
      {audit_html}
      <div class="page-footer">Mọi nguồn nội bộ đều được chuẩn hóa thành nhãn công khai.</div>
    </section>

    <section class="report-page">
      <div class="section-kicker">Chỉ tiêu</div>
      <h2 class="section-title">Chỉ tiêu trọng yếu</h2>
      {glossary_html}
      {metric_table_html}
    </section>

    <section class="report-page">
      <div class="section-kicker">Biểu đồ</div>
      <h2 class="section-title">Biểu đồ có thể đọc</h2>
